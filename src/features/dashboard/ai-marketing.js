export function buildAggregatePayload(model = {}, crm = {}) {
  ...
  const issues = model.recommendation?.issues || [];  const aggregateCrm = ["leads","booked","arrived"].reduce((out, key) => { const rows = Array.isArray(crm?.[key]) ? crm[key] : []; out[key] = { count: rows.length, byService: rows.reduce((m, r) => { const service = String(r.service || "Khong xac dinh").trim(); m[service] = (m[service] || 0) + 1; return m; }, {}) }; return out; }, {});
