export function buildAggregatePayload(model = {}, crm = {}) {
  ...
  const issues = model.recommendation?.issues || [];  const aggregateCrm = ["leads","booked","arrived"].reduce((out, key) => { const rows = Array.isArray(crm?.[key]) ? crm[key] : []; out[key] = { count: rows.length, byService: rows.reduce((m, r) => { const service = String(r.service || "Khong xac dinh").trim(); m[service] = (m[service] || 0) + 1; return m; }, {}), byStatus: rows.reduce((m, r) => { const status = String(r.status || "Khong xac dinh").trim(); m[status] = (m[status] || 0) + 1; return m; }, {}), byDate: rows.reduce((m, r) => { const date = String(r.aptDate || r.date || "").slice(0, 10); if (date) m[date] = (m[date] || 0) + 1; return m; }, {}) }; return out; }, {});
